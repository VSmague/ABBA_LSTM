...
def train_abba_rolling(
    ...
    """
    Entraîne un ABBALSTM sur une série symbolique avec validation rolling window
    """
    if model.stateful and batch_size != 1:
        raise ValueError("Stateful LSTM requires batch_size=1")
    ...
    if train_size is None:
        train_size = len(series) - val_size
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for train_fold, _ in folds:
            X_train, y_train = create_lagged_series_symbolic(train_fold, lag)
            X_train, y_train = X_train.to(device), y_train.to(device)
        ...
        with torch.no_grad():
            for _, val_fold in folds:
                X_val, y_val = create_lagged_series_symbolic(val_fold, lag)
                X_val, y_val = X_val.to(device), y_val.to(device)
